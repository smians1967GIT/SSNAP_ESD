...
def extract_metric(sheet_name, metric_id):
    ...
    try:
        ...
        if metric_row.empty:
            return f"❌ Metric ID '{metric_id}' not found in sheet '{sheet_name}'", None
...
def gradio_interface(sheet_name, metric_id):
    message, file_path = extract_metric(sheet_name, metric_id)
...
demo = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Enter Metric ID (e.g., L32.3)")
    ],
    outputs=[
    ...
    description="Enter the sheet name and metric ID to extract and clean outcome data for export."
)
